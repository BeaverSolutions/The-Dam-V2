...
describe('auto approval recovery gates', () => {
  it('passes a seasoned high-score verified email with no recent send', async () => {
    const prior = process.env.AUTO_APPROVAL_RECOVERY_ENABLED;
    process.env.AUTO_APPROVAL_RECOVERY_ENABLED = 'true';
    try {
      await expect(gatePendingMessage(baseRow)).resolves.toEqual({ pass: true, reason: null });
    } finally {
      if (prior === undefined) delete process.env.AUTO_APPROVAL_RECOVERY_ENABLED;
      else process.env.AUTO_APPROVAL_RECOVERY_ENABLED = prior;
    }
  });

  it('requires AUTO_APPROVAL_RECOVERY_ENABLED to be explicitly true', async () => {
    const prior = process.env.AUTO_APPROVAL_RECOVERY_ENABLED;
    delete process.env.AUTO_APPROVAL_RECOVERY_ENABLED;
    try {
      await expect(gatePendingMessage(baseRow)).resolves.toEqual({
        pass: false,
        reason: 'AUTO_APPROVAL_RECOVERY_ENABLED not true (manual approval required)',
      });
    } finally {
      if (prior === undefined) delete process.env.AUTO_APPROVAL_RECOVERY_ENABLED;
      else process.env.AUTO_APPROVAL_RECOVERY_ENABLED = prior;
