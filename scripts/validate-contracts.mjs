...
const autoApprovalRecoveryGuarded = autoApprovalRecovery.includes("AUTO_APPROVAL_RECOVERY_ENABLED === 'true'")
  && index.includes("AUTO_APPROVAL_RECOVERY_ENABLED !== 'true'")
...
const hourlyReportTruth = hourlyReport.includes('/api/autonomous/system-health')
  ...
  && hourlyReport.includes('<b>Action needed</b>')
  && hourlyReport.includes('KPI-gap:')
  && hourlyReport.includes('daily kickoff disabled')
  && hourlyReport.includes('sent leads missing follow-up rows')
